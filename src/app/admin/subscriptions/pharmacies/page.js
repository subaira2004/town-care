...
import { Loader2, Search, Building2, Check, X, Calendar, DollarSign, AlertCircle, TrendingUp } from 'lucide-react';
...
export default function AdminSubscriptionsPharmaciesPage() {
  ...
  const [tab, setTab] = useState('active'); // 'active', 'trial', 'cancelled'

  const { data: pharmacies, isLoading: pharmaciesLoading, mutate } = useSWR(
    `admin-pharmacies-subscriptions-${tab}`,
    async () => {
      let query = supabase
        .from('pharmacies')
        .select(`
  ...
  const handleAssignSubscription = async (e) => {
    ...
    try {
      ...
      if (existing) {
        ...
        // Insert new
        const { error } = await supabase.from('pharmacy_subscriptions').insert([subscriptionData]);
        if (error) throw error;
      }
  ...
  const handleMarkPayment = async (pharmacyId, amount) => {
    if (!confirm(`Mark ₹${amount} as received for this pharmacy's current invoice?`)) return;
  ...
  const filteredPharmacies = pharmacies?.filter(pharm => {
    ...
    if (tab === 'cancelled') return !sub || sub.status === 'cancelled' || sub.status === 'expired';
    return true;
  }) || [];
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', backgroundColor: 'var(--surface-hover)', padding: '0.25rem', borderRadius: 'var(--radius-md)', width: 'fit-content' }}>
            ...
            <tbody>
              {filteredPharmacies.map(pharm => {
                ...
                const usagePercent = plan?.max_tokens_per_month > 0
                  ? Math.round((sub?.tokens_used_this_month / plan.max_tokens_per_month) * 100)
                  : 0;
                ...
                return (
                  <tr key={pharm.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '1rem' }}>
                      ...
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{pharm.phone}</div>
                    </td>
                    <td style={{ padding: '1rem' }}>
                    ...
                    <td style={{ padding: '1rem' }}>
                      {plan ? (
                        <div>
                          ...
                          <div style={{ fontSize: '0.75rem', color: 'var(--secondary)' }}>₹{plan.price_monthly}/mo</div>
                        </div>
                      ) : (
                        <span style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>No subscription</span>
                      )}
                    </td>
                    <td style={{ padding: '1rem' }}>
                      ...
                      {plan?.max_tokens_per_month > 0 ? (
                        <div>
                          <div style={{ fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                            {sub?.tokens_used_this_month || 0} / {plan.max_tokens_per_month}
                          </div>
                          <div style={{ height: '6px', backgroundColor: 'var(--border)', borderRadius: '3px', overflow: 'hidden' }}>
                            <div
                              style={{
                                height: '100%',
                                width: `${usagePercent}%`,
                                backgroundColor: usagePercent > 80 ? 'var(--danger)' : usagePercent > 50 ? 'var(--warning)' : 'var(--secondary)',
                        ...
                        <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Unlimited</span>
                      )}
                    </td>
                    <td style={{ padding: '1rem', textAlign: 'right' }}>
                      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                        <button
                          onClick={() => { setSelectedPharmacy(pharm); setShowAssignModal(true); }}
                        ...
                        {sub && (
                          <button
                            onClick={() => handleMarkPayment(pharm.id, plan?.price_monthly || 0)}
      ...
      {showAssignModal && selectedPharmacy && (
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: '500px', padding: '2rem', position: 'relative' }}>
            <button
              onClick={() => { setShowAssignModal(false); setSelectedPharmacy(null); }}
            ...
            <h2 style={{ fontSize: '1.25rem', marginBottom: '1.5rem' }}>
              {selectedPharmacy.pharmacy_subscriptions?.[0] ? 'Manage Subscription' : 'Assign Subscription'}
            </h2>
            ...
            <form onSubmit={handleAssignSubscription}>
              <div className="form-group">
                ...
                <select name="plan_id" className="form-select" required defaultValue={selectedPharmacy.pharmacy_subscriptions?.[0]?.plan_id || ''}>
                  <option value="">Select a plan...</option>
                  {plans.map(plan => (
                    <option key={plan.id} value={plan.id}>
                      {plan.name} - ₹{plan.price_monthly}/month ({plan.max_tokens_per_month || '∞'} tokens)
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  type="date"
                  className="form-input"
                  defaultValue={selectedPharmacy.pharmacy_subscriptions?.[0]?.current_period_start || new Date().toISOString().split('T')[0]}
                  required
                />
              </div>
              ...
              <div className="form-group">
                ...
                <select name="period_months" className="form-select" defaultValue="1">
              ...
              >
                {loading ? <Loader2 className="animate-spin" size={20} /> : 'Save Subscription'}
              </button>
            </form>
          </div>
        </div>
